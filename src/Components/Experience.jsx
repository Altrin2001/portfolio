...
const Experience = () => {
  return (
    ...
    >
      ...
      {/* === LAPTOP VIEW (md and up) === */}
      <div className="hidden md:flex relative w-full max-w-5xl mx-auto aspect-[16/10]">
        {/* Laptop Frame Image */}
        <img
          src="/laptop.png" // place your realistic laptop image in public folder
          alt="Laptop"
          className="w-full h-auto object-contain pointer-events-none select-none"
        />

        {/* Laptop Screen Overlay Content */}
        <div className="absolute top-[11%] left-[12.5%] w-[75%] h-[61%] bg-gradient-to-br from-gray-800 via-gray-900 to-gray-800 rounded-xl p-4 overflow-y-auto space-y-4 shadow-inner flex flex-col">
          {/* Animated Heading inside laptop */}
          ...
          {experiences.map((exp, idx) => (
            <div
              key={idx}
              className="bg-black bg-opacity-60 text-white rounded-lg p-4 border-l-4 border-blue-500 shadow-md transition-transform hover:scale-[1.01]"
            >
      ...
      {/* === MOBILE VIEW (below md) with CRT style === */}
      <div className="flex md:hidden flex-col w-full max-w-md mx-auto gap-6 p-4 bg-gradient-to-br from-black via-gray-900 to-black border-t-2 border-b-2 border-green-500/30 relative overflow-hidden">
        ...
        {experiences.map((exp, idx) => (
          ...
          >
            <div className="mb-2 flex flex-col gap-1">
              <h3 className="text-base font-bold group-hover:text-green-200 transition">{exp.company}</h3>
              <p className="text-xs text-green-300/80">
                <span className="animate-pulse">{exp.role}</span> | Duration: {exp.duration}
              </p>
            </div>
